...
export function Header() {
  ...
  return <header className="w-full h-16 px-4 flex items-center justify-between bg-background border-b">
      {/* Left section: Logo and Account Switch */}
      <div className="flex items-center gap-4">
        <div className="text-2xl font-bold">TradeVult</div>
      ...
      <div className="flex-1 px-6 max-w-xl mx-auto">
        <div className="relative">
          <Input type="search" placeholder="Search trades, tags, symbols..." className="w-full pl-10" />
          <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={handleThemeToggle} className="rounded-full" title={`Switch to ${theme === 'light' ? 'dark' : 'light'} mode`}>
          {theme === "light" ? <Moon size={20} /> : <Sun size={20} />}
        </Button>
        
        <Button variant="ghost" size="icon" className="rounded-full relative">
          <Bell size={20} />
          {notifications > 0 && <span className="absolute top-1 right-1 flex h-2 w-2 items-center justify-center rounded-full bg-destructive" />}
        </Button>
